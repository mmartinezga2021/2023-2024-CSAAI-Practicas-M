...
btnMinPath.onclick = () => {
  ...
  document.getElementById('tiempoTotal').textContent = "Tiempo total de retardo: " + tiempoTotal.toFixed(3) + " ms";
...
btnCNet.onclick = () => {
  ...
  // Limpiamos el canvas
  // Display the total delay time

  // Redraw the network with the minimum path
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  drawNet(redAleatoria, rutaMinimaConRetardos);
